...
app = Flask(__name__)
def obtener_mensaje_transferencia():
    banco = os.environ.get("TRANSFERENCIA_BANCO")
    clabe = os.environ.get("TRANSFERENCIA_CLABE")
    beneficiario = os.environ.get("TRANSFERENCIA_BENEFICIARIO")

    if not banco or not clabe or not beneficiario:
        return None

    return (
        "Puedes realizar tu transferencia a:\n\n"
        f"Banco: {banco}\n"
        f"CLABE: {clabe}\n"
        f"Beneficiario: {beneficiario}\n\n"
        "Cuando la realices, envíanos el comprobante."
    )
